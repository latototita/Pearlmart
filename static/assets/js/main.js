...
(function ($) {
	...
	//  brand Menu
	/*-- Variables --*/
	var brandToggleWrap = $('.brand-toggle-wrap');
	var brandToggle = $('.brand-toggle');
	var brandMenu = $('.brand-menu');

	/*-- brand Menu Toggles --*/
	function brandSubMenuToggle() {
		var screenSize = $window.width();
		if (screenSize <= 991) {
			$('.brand-menu .menu-item-has-children > a').prepend('<span class="expand menu-expand"></span>');
			$('.brand-menu .menu-item-has-children ul').slideUp();
		} else {
			$('.brand-menu .menu-item-has-children > a .menu-expand').remove();
			$('.brand-menu .menu-item-has-children ul').slideDown();
		}
	}
	...
	// brand Sub Menu
	$('.brand-menu').on('click', 'li a, li a .menu-expand', function (e) {
		var $a = $(this).hasClass('menu-expand') ? $(this).parent() : $(this);
		if ($a.parent().hasClass('menu-item-has-children')) {
			if ($a.attr('href') === '#' || $(this).hasClass('menu-expand')) {
				if ($a.siblings('ul:visible').length > 0) $a.siblings('ul').slideUp();
